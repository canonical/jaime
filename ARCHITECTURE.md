...
collect previous-instance logs for containers that have restarted
collect Kubernetes events for the pod
collect CPU/memory usage from metrics-server, if present
summarise the pod (phase, node, conditions, containers with current and last state, init containers, restarts, probes, resources, volumes)
...
## Log files                    plan-driven log tails
## Processes                    plan-driven, or ps aux as fallback
## Systemd units                plan units, or "Failed systemd units" as fallback
...
## Kubernetes pod               containers with current and last state, init containers
## Previous container logs      only for containers that have restarted
## Kubernetes events
## Resource usage (metrics-server)
## Juju application config
-- both substrates --
## Recent unit logs             error/warning lines with a context window
```
...
`tests/unit/test_examples.py` regenerates the committed example and fails if a
heading changes. `examples/report.md` is generated by `make examples`, never
edited by hand.
